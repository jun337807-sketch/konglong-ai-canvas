...
export function UserAdminDialog({
  ...
  const handleAddUser = async () => {
    ...
    await userRepository.create({
      ...
      role: 'user',
      permissions: { workspace: true, canvas: true, isAdmin: false }
    });
    await refreshUsers();
  ...
  const togglePermission = async (username: string, field: 'canvas' | 'isAdmin') => {
    const user = usersDB.find(u => u.username === username);
    if (!user) return;
    const currentPermissions = user.permissions || { workspace: true, canvas: true, isAdmin: false };
    const nextPermissions = { ...currentPermissions, [field]: !currentPermissions[field] };

    await userRepository.updateByUsername(username, {
      permissions: nextPermissions,
      role: nextPermissions.isAdmin ? 'admin' : 'user'
    });
    await refreshUsers();
  };
    ...
    <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-6" onClick={onClose}>
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-4xl max-h-[80vh] flex flex-col overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
          ...
          {usersDB.map((user) => (
            <div key={user.id || user.username} className="bg-zinc-950 border border-zinc-800/80 rounded-xl p-4 flex flex-col gap-4">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-3 border-b border-zinc-800/50 gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-zinc-500">最后登录：{user.lastLogin || '从未登录'}</div>
                  </div>
                </div>
